...
def mingd(model, X, y, args, target):
    ...
    for t in range(args.num_iter):        
        ...
        preds = model(X_r + delta_r)
        loss = loss_mingd(preds, target[remaining])
...
def get_random_label_only(args, loader, model, num_images = 1000):
    ...
    lp_d = [torch.cat(lp_dist[i], dim = 0).unsqueeze(-1) for i in range(1)]    
    ...
    for i,batch in enumerate(loader):
        if args.regressor_embed == 1: ##We need an extra set of `distinct images for training the confidence regressor
            if(ex_skipped < num_images):
                ...
                continue
        # for j,distance in enumerate(["linf", "l2", "l1"]):
        for j,distance in enumerate(["linf"]):
            temp_list = []
            for target_i in range(args.num_classes):
                X,y = batch[0].to(device), batch[1].to(device) 
                args.distance = distance
                # args.lamb = 0.0001
                delta = mingd(model, X, y, args, target = y)
                yp = model(X+delta) 
                distance_dict = {"linf": norms_linf_squeezed, "l1": norms_l1_squeezed, "l2": norms_l2_squeezed}
                distances = distance_dict[distance](delta)
                temp_list.append(distances.cpu().detach().unsqueeze(-1))
...
def feature_extractor(args, config):
    ...
    if args.independent==True:
        ...
        with open(f"{args.save_dir}/models/{args.dataset}/model_ind/log.txt","a+") as f:
            f.write("Test: {}\n".format(test_acc))
        torch.save(student.state_dict(), f"{args.save_dir}/models/{args.dataset}/model_ind/final.pt")
    if args.Amazon == True:
        import boto3
    ...
    if args.finetune:
        ...
        with open(f"{args.save_dir}/models/{args.dataset}/model_fine-tune/log.txt","a+") as f:
            f.write("Test: {}\n".format(test_acc))
    ...
    if args.adv_gen:
        ...
        torch.save(watermarkloader_a, f"{args.save_dir}/models/{args.dataset}/model_teacher/wm_loader.pt")
    # _, train_acc  = epoch_test(args, train_loader, student)
    else:
        watermarkloader_a = torch.load(f"{args.save_dir}/models/{args.dataset}/model_teacher/wm_loader.pt")
...
def parse_args():
    ...
    parser.add_argument('-c', '--config', type=str, default='configs/cifar10/train/resnet_di.yaml',
                        help="Path to config file. Determines all training params.")
    parser.add_argument('--save_dir', type=str, default="defences/cifar10/DI")     
    parser.add_argument('--model_id', type=str, default='teacher')
    parser.add_argument('--feature_type', type=str, default='mingd')
    parser.add_argument('--adv_gen', type=bool, default=True)
