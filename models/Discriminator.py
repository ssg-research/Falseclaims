...
class DiscriminatorNet_CelebA(nn.Module):
    def __init__(self, nc=3, nhf=8, output_function=nn.Sigmoid):
        super(DiscriminatorNet_CelebA, self).__init__()
        #self.key_pre = RevealPreKey()
        # input is (3+1) x 256 x 256
        self.main = nn.Sequential(
            nn.Conv2d(nc, nhf, 4, 2, 1),
...
class DiscriminatorNet_ImageNet(nn.Module):
    def __init__(self, nc=3, nhf=8, output_function=nn.Sigmoid):
        super(DiscriminatorNet_ImageNet, self).__init__()
        #self.key_pre = RevealPreKey()
        # input is (3+1) x 256 x 256
        self.main = nn.Sequential(
            nn.Conv2d(nc, nhf, 4, 2, 1),
